...
namespace TUIO2 {
    ...
    public:
        ...
        void setSourceString(std::string src_string) {
            ...
            // updated strdup to _strdup
            char *data = _strdup(src_string.c_str());

            // Updated strtok to strtok_s; needed a next token argument
            char *nexttoken;

            char *name_inst = strtok_s(data, "@", &nexttoken);

            char *addr = strtok_s(NULL, "@", &nexttoken);
            if (addr!=NULL) source_address = std::string(addr);
            else source_address = (char*)"0x7F000001";
            
            char *name = strtok_s(name_inst, ":", &nexttoken);
            source_name = std::string(name);
            
            char *inst = strtok_s(NULL, ":", &nexttoken);
